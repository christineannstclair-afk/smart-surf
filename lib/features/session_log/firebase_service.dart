...
class FirebaseService {
  ...
  /// Returns detailed info about insights generated by the user on a specific UTC day.
  Future<Map<String, dynamic>> getDailyInsightInfo(String utcDayKey) async {
    await _ensureReady();
    final uid = currentUid;
    if (uid == null) return {'count': 0, 'sessionIds': []};
    ...
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        return {
          'count': data?['insightCount'] as int? ?? 0,
          'sessionIds': data?['sessionIds'] as List<dynamic>? ?? [],
        };
      }
    } catch (e) {
      debugPrint("Error fetching daily insight info: $e");
    }
    return {'count': 0, 'sessionIds': []};
  }

  @Deprecated('Use getDailyInsightInfo instead')
  Future<int> getDailyInsightCount(String utcDayKey) async {
    final info = await getDailyInsightInfo(utcDayKey);
    return info['count'] as int;
  }

  /// Increments the daily insight counter and tracks the session ID.
  Future<void> incrementDailyInsightCount(String utcDayKey, String sessionId) async {
    await _ensureReady();
    final uid = currentUid;
    if (uid == null) return;
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .doc(utcDayKey)
          .set({
        'insightCount': FieldValue.increment(1),
        'sessionIds': FieldValue.arrayUnion([sessionId]),
        'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true));
    } catch (e) {
      debugPrint("Error incrementing daily insight count: $e");
    }
  }
}
